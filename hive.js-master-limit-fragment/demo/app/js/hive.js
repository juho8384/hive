...
var Overlay = {};
Overlay.Choking = 3e4, Overlay.MaxPartners = 15;
var Discovery = {},
    service;
...
var ByteBuffer = dcodeIO.ByteBuffer,
    ...
    Delfrag = protoFile.build("Delfrag"), //delfrag
...
var peerstatic = {};
var point1 = 375; // point of speed 
var fragmentLimit = 400;
var debugPram = 1;
var fromwhoPram = 1;
...
Utils.genFragmentId = function(a, b) {
    ...
    var g = function() {
        var a = new Promise(function(a, b) {
            d.channel.onopen = function() {
                ...
                Peer.peerstatic[d.remotePeer] = {}, 
                Peer.peerstatic[d.remotePeer].totaltime = 0,
                Peer.peerstatic[d.remotePeer].priority = 3,  //statics
                Peer.peerstatic[d.remotePeer].count = 0; 
    ...
    return b.echo = function() {
        ...
        return f(c)
    }, b.fromwho = function(a, fragmentID, remotePeerguid) {
        var p2pbytes = Metrics.summary.traffic.p2pSubRespQt || 0,
            srcbytes = Metrics.summary.traffic.srcRespQt || 0;
        var priorities = {};
        for ( var i2 in a.peerstatic){
            priorities[i2]  = a.peerstatic[i2].priority;
        }
        var save = p2pbytes / (p2pbytes + srcbytes + 1) * 100;
        var c = b.event("debugging", {
              guid: a.guid,
...
}(), FragmentCache = function() { //delfrag
   var a = {};
        a.havingfragmentNum = 0;
        a.totalfragmentNum = 0;
        a.idx2 = {};
    return a.idx = {}, a.putFragment = function(b, c) { 
        var d = Utils.genFragmentId(b.url, b.range),
            e = new Fragment(b, c);
            console.log("**put fragment :" + a.havingfragmentNum);
            if(a.havingfragmentNum >= fragmentLimit){
                   var fragmentId = Object.keys(a.idx2)[0];
                console.log("**too much fragment")
                console.log("**delete : " + fragmentId);
                delete a.idx2[fragmentId];
                Peer.sendDelfrag(fragmentId);
                setTimeout( function(){
                     delete a.idx[fragmentId];
                   a.havingfragmentNum -= 1;
                },5000);
            }
            a.havingfragmentNum += 1;
            a.totalfragmentNum += 1;
            a.idx[d] = e;
            a.idx2[d] = 0;
        return d;
...
ObjectIndex = function() { //delfrag
    ...
    return a.idx = {}, a.put = function(b, c) {
        a.contains(b) || (a.idx[b] = []), a.idx[b].push(c)
            ...
            var count = 0;
            if( b in a.idx){
                console.log("**have fragement index");
                var d = a.idx[b];
                console.log("**received delfrag and delete "+ b + c);
             Utils.removeArrayItem(d, c);
             for ( var i in a.idx[b] ){
                count +=1;
              }
              if(count == 0 ){
                console.log("**idx[b] was deleted");
                delete a.idx[b];
              }
            }
            else ("**no ObjectIndex");
    ...
    return b.channel = a, b.pendingRequests = {}, b.inMsg = null, b.outMsg = null, b.pendingOutRequests = [], b.sendHave = function(a) {
    ...
    },  b.sendDelete = function(fragmentId) { //delfrag
        console.debug("**send Delete Msg");
        var delfrag = new Delfrag(fragmentId);
        b.sendMessage(Transport.MsgTypes.Delfrag,delfrag);
    },  b.sendPing = function() {
        ...
        console.info("TIMED OUT " + a);
        if(Peer.peerstatic[b.channel.remotePeer].priority <= 2){
                Peer.peerstatic[b.channel.remotePeer].priority = 1;
            }
        else Peer.peerstatic[b.channel.remotePeer].priority -= 2;
        console.log("**speed error : " + Peer.peerstatic[b.channel.remotePeer].priority);
        b.stopTransfer(a, "TIMED OUT");
        ...
        switch (c.msgId) {
            case Transport.MsgTypes.Fragment: //statics
                var d = Response.decode(c.rcvMesgData);
                var rcvtime = new Date;
                Peer.peerstatic[b.channel.remotePeer].fragmentTime = rcvtime.getTime() - Peer.peerstatic[b.channel.remotePeer].timer2;
                c.fragmentData = d.data;
                var bytes = c.fragmentData.toArrayBuffer();
                if (bytes.byteLength < 100000){
                    console.log("** bytes is low :" + bytes.byteLength)
                    delete b.pendingRequests[c.transferId], Peer.deliverFragment(c);
                    break;
                }
                var speed = bytes.byteLength/(Peer.peerstatic[b.channel.remotePeer].fragmentTime); //KB unit
                console.log("** bytes is high :" + bytes.byteLength);
                if( speed >= point1 ){
                      if(Peer.peerstatic[b.channel.remotePeer].priority >= 4){
                        Peer.peerstatic[b.channel.remotePeer].priority = 5;
                      }
                      else Peer.peerstatic[b.channel.remotePeer].priority += 1;

                      console.log("**speed: " + speed + " > point1: " + Peer.peerstatic[b.channel.remotePeer].priority);

                }
                else  if ( speed < point1){
                      if(Peer.peerstatic[b.channel.remotePeer].priority <= 2){
                        Peer.peerstatic[b.channel.remotePeer].priority = 1;
                      }
                      else Peer.peerstatic[b.channel.remotePeer].priority -= 1;
                      console.log("**speed: " + speed + "  < point2: " + Peer.peerstatic[b.channel.remotePeer].priority);
                }
                //Peer.fromwho(b.channel.remotePeer);
                delete b.pendingRequests[c.transferId], Peer.deliverFragment(c);
                break;
            ...
            case Transport.MsgTypes.Pong:
                    ...
                    var rcvtime = new Date;
                    Peer.peerstatic[b.channel.remotePeer].totaltime += rcvtime.getTime() - Peer.peerstatic[b.channel.remotePeer].timer;
                    Peer.peerstatic[b.channel.remotePeer].count +=1;
                    console.debug("**receive pong by " + b.channel.remotePeer + " totaltime: " + Peer.peerstatic[b.channel.remotePeer].totaltime + ", count: " + Peer.peerstatic[b.channel.remotePeer].count);
                break;   
            case Transport.MsgTypes.Delfrag:  //delfrag
                    var delfrag1 = Delfrag.decode(c.rcvMesgData);
                    ObjectIndex.Delete(delfrag1.fragmentId,a.remotePeer);
                    // delete has fragment id 
                break;                     
...
var Peer = function() {
    ...
    }, a.fromwho = function(remotePeerguid,fragmentID) {
       if (fromwhoPram== 1){
        Discovery.fromwho(a, fragmentID, remotePeerguid);
        }
    }, a.init = function() {
        return setInterval(a.update, 15e3), a.update()
        ...
        if (a.isSendHaves)
            ...
            console.log("**sendpingpong work");
            for(var i=0; i<Object.keys(a.connections).length; i++){
                        console.log("**sendpingpong start");
                      var channel = a.connections[Object.keys(a.connections)[i]];
                      var sendingTime = new Date;
                      a.peerstatic[Object.keys(a.connections)[i]].timer = sendingTime.getTime();
                     console.log("**sendpin to "+ Object.keys(a.connections)[i]);
                      channel.p2pTransport.sendPing();
             }
            //setTimeout(a.sendPingPong,4e3);
    }, a.sendDelfrag = function(fragmentId){ //delfrag
             console.log("**send delete message work");
            for(var i=0; i<Object.keys(a.connections).length; i++){
                        console.log("**send delete start");
                      var channel = a.connections[Object.keys(a.connections)[i]];
                      console.log("**sendpin to "+ Object.keys(a.connections)[i]);
                      channel.p2pTransport.sendDelete(fragmentId);
             }
        ...
        try {
            if ("undefined" != typeof d) {
                var e = _.filter(d, function(b) {
                        ...
                        return !1
                    }); // e is array, having fragment

                if (console.debug("CHOICE ", e), e.length > 0) {
                    var g ; 
                    if( b.fragmentId[47] == 'a' ) {
                        var maxPriority = -1;
                        var list = [];
                        for( var i in e){
                            if( maxPriority < a.peerstatic[e[i]].priority ) maxPriority = a.peerstatic[e[i]].priority;
                        }
                        for( var i2 in e){
                            if( maxPriority-2 <= a.peerstatic[e[i2]].priority) list.push(e[i2]) ;
                            console.log("** maxPri: " + maxPriority + ", Peer pri: " + a.peerstatic[e[i2]].priority);
                        }
                        if( list.length == 1) var g = list[0];
                        else{
                            if(a.last2 != -1 )  list.splice(list.indexOf(a.last2),1);
                        ...
                        if( list.length == 1) var g = list[0];
                            ...
                            if(a.last != -1 )  list.splice(list.indexOf(a.last),1);
                            g = list[Math.floor(Math.random()*list.length)];

                        }
                        a.last = g;
                    }
		    a.fromwho(g,b.fragmentId);
                    console.log("**send request to " + g);
                    if (g in a.connections) {
                        var h = a.connections[g];
                        var requeststartTime = new Date;
                        a.peerstatic[g].timer2 = requeststartTime.getTime(),
                        a.peerstatic[g].fragmentTime = 0,
                        console.debug("SEND REQUEST FOR " + c + " TO " + g + " CHANNEL " + h + " " + h.p2pTransport), b.channel = h, h.p2pTransport.sendRequest(b)
