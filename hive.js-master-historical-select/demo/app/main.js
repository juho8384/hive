...
app.controller('DashController', function($scope, Sources, Notes, Contributors, PlayerLibraries, ShowcaseLibraries) {
    ...
    function metricChanged(e) {
        ...
        if (e.data.stream == "video") {
            ...
            if (metrics) {
                ...
                Metrics.record('metrics', 'videoAvgBufQt', parseFloat(metrics.bufferLengthValue))
                Metrics.increase('metrics', 'videoBufN', parseInt(metrics.droppedFramesValue))


                metrics = getCribbedMetricsFor("audio");
                $scope.audioBufferLength = metrics.bufferLengthValue;
                BL = $scope.videoBufferLength > $scope.audioBufferLength ? $scope.audioBufferLength : $scope.videoBufferLength;
                $scope.mediaBufferLength = $scope.videoBufferLength > $scope.audioBufferLength ? $scope.audioBufferLength : $scope.videoBufferLength;
            }
        }
        ...
        if (e.data.stream == "audio") {
            ...
            if (metrics) {
                ...
                Metrics.record('metrics', 'audioAvgBufQt', parseFloat(metrics.bufferLengthValue))
                Metrics.increase('metrics', 'audioBufN', parseInt(metrics.droppedFramesValue))
    ...
    $scope.doLoad = function (videoQuality, audioQuality) {
        player.attachSource($scope.selectedItem.url);

        console.debug("Audio/video quality", videoQuality, audioQuality)
        if(typeof videoQuality !== 'undefined' || typeof audioQuality !== 'undefined') {
            player.setAutoSwitchQuality(false)
        }

        if(typeof videoQuality !== 'undefined') {
            player.setQualityFor('video', videoQuality)
        }

        if(typeof audioQuality !== 'undefined') {
            player.setQualityFor('audio', audioQuality)
        }
    }
	...
	if (vars && vars.hasOwnProperty("haves")) {
    	console.debug("SENDING HAVES "+vars.haves);
        Peer.isSendHaves = vars.haves;
    }
    ...
    if (paramUrl !== null) {
      ...
      var videoQuality, audioQuality;

      if(vars.hasOwnProperty("audio_quality")) {
          audioQuality = parseInt(vars.audio_quality)
      }

      if(vars.hasOwnProperty("video_quality")) {
          videoQuality = parseInt(vars.video_quality)
      }

    	if (startPlayback) {
          if(vars.hasOwnProperty("playback_delay")) {
              var delay = parseInt(vars.playback_delay)
              setTimeout(function() {
                $scope.doLoad(videoQuality, audioQuality)
              }, delay)
          } else {
              $scope.doLoad(videoQuality, audioQuality)
          }
